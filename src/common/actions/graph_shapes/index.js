...
import { infobox, graph_viz, graphioGremlin, utils} from 'actions'
	...
	function add_checkbox_prop(item,selected_items){
		...
		for (var prop_idx in item_properties){
			...
			if((!d3.select("#"+prop_id).empty()) && d3.select("#"+prop_id).property("checked")){
				utils.attach_property(selected_items,prop_name,prop_id_nb,item);
			}
		}
	}
	...
	function colorize(prop_name){
		...
		if (prop_name =="none"){
			...
			node_code_color = d3.scale.ordinal().domain(value_set).range(d3.range(0,value_set.size));
			d3.selectAll(".base_circle").style("fill",function(d){
				return color_palette(node_code_color(d.label));
			});
			d3.selectAll(".Pin").style("fill",function(d){
			...
			node_code_color = d3.scale.ordinal().domain(value_set).range(d3.range(0,value_set.size))//value_set.length])
			d3.selectAll(".base_circle").style("fill",function(d){
				if (typeof d.properties[prop_name] !=="undefined"){
	...
	export	 { show_names,decorate_node,decorate_link,node_color,
		node_size,edge_color,node_stroke_width,create_edge_label,
		decorate_old_elements,colorize};
